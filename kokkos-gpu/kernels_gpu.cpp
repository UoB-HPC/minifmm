    ...
    t_view& d_ax, t_view& d_ay, t_view& d_az, t_view& d_p)
{
    ...
    for (size_t i = tid; i < t_num_points+31; i += tsz)
    {
        TYPE ax = 0.0, ay = 0.0, az = 0.0, p = 0.0;
        const TYPE xi = (i >= t_num_points) ? 0.0f: tx[i];
        const TYPE yi = (i >= t_num_points) ? 0.0f: ty[i]; 
        const TYPE zi = (i >= t_num_points) ? 0.0f: tz[i];
    ...
    t_view& d_ax, t_view& d_ay, t_view& d_az, t_view& d_p)
{
    ...
    __shared__ float4 shmem_base[shmem_sz*4];
    float4* shmem = &shmem_base[shmem_sz*warp_id];

    long tid = member.team_rank();
    long tsz = member.team_size();

    //Kokkos::parallel_for(Kokkos::TeamThreadRange(member, 0, t_num_points), [&](int i)
    for (size_t i = tid; i < t_num_points+31; i += tsz)
    {
        TYPE ax = 0.0, ay = 0.0, az = 0.0, p = 0.0;
        const TYPE xi = (i >= t_num_points) ? 0.0f: tx[i];
        ...
        for (long jb = 0; jb < s_num_points; jb += shmem_sz)
        {
            ...
            {
                if (j+jb >= s_num_points) break;
                const TYPE dx = shmem[j].x - xi;
                const TYPE dy = shmem[j].y - yi;
                const TYPE dz = shmem[j].z - zi;
                const TYPE wj = shmem[j].w;
                const TYPE r = dx*dx + dy*dy + dz*dz;
                //TYPE inv_r = TYPE_ONE/TYPE_SQRT(r);
                TYPE inv_r = rsqrtf(r);
                const TYPE t = wj*inv_r;
