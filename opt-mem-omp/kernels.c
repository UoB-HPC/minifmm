...
void p2p_intrinsics(t_fmm_options* options, t_node* target, t_node* source)
{
    ...
    size_t i;
    size_t lim = (t_num_points < 4) ? 0 : t_num_points - 4;
    for (i = 0; i < lim; i += 4)
    {
        __m256d ax = _mm256_setzero_pd(), ay = _mm256_setzero_pd(), az = _mm256_setzero_pd(), p = _mm256_setzero_pd();
        __m256d xi = _mm256_load_pd(tx+i);
        __m256d yi = _mm256_load_pd(ty+i);
        __m256d zi = _mm256_load_pd(tz+i);
        ...
        __m256d tmp_tax = _mm256_load_pd(&tax[i]);
        tmp_tax = _mm256_add_pd(tmp_tax, ax);
        _mm256_store_pd(&tax[i], tmp_tax);

        __m256d tmp_tay = _mm256_load_pd(&tay[i]);
        tmp_tay = _mm256_add_pd(tmp_tay, ay);
        _mm256_store_pd(&tay[i], tmp_tay);

        __m256d tmp_taz = _mm256_load_pd(&taz[i]);
        tmp_taz = _mm256_add_pd(tmp_taz, az);
        _mm256_store_pd(&taz[i], tmp_taz);

        __m256d tmp_tp = _mm256_load_pd(&tp[i]);
        tmp_tp = _mm256_add_pd(tmp_tp, p);
        _mm256_store_pd(&tp[i], tmp_tp);
    }
